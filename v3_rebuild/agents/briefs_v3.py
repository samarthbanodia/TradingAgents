...
def compute_v3_features(row, ohlcv_df, baselines):
    ...
    cluster_bars = _get_cluster_bars(ticker, t0, cluster_len, ohlcv_df)
    ...
    # 3. Signed volume + VPIN over cluster
    sv_out = _compute_signed_volume(cluster_bars)
    features["net_signed_vol"]   = sv_out["net_signed_vol"]
    features["signed_vol_ratio"] = sv_out["signed_vol_ratio"]
    features["vpin_proxy"]       = _compute_vpin_proxy(cluster_bars)
